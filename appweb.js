...
function displaySearchList(items) {
    ...
    items.forEach(item => {
        ...
        div.style.cssText = "padding:15px; cursor:pointer; border-bottom:1px solid #f3f4f6; display:flex; justify-content:space-between; align-items:center;";
        ...
        div.onclick = () => {
            ...
            document.querySelector('.surah-select span').innerText = `${item.number}. ${item.englishName}`;
            
            // Sedd kolchi
            togglePopup('search-sidebar', 'close');
            document.getElementById('surah-sidebar-nav').classList.remove('open-flex');
            document.getElementById('verse-sidebar-nav').classList.remove('open-flex');
...
};document.getElementById('close-search').onclick = () => togglePopup('search-sidebar', 'close');
document.getElementById('open-languages').onclick = () => togglePopup('language-sidebar', 'open');
document.getElementById('close-languages').onclick = () => togglePopup('language-sidebar', 'close');
document.getElementById('open-settings').onclick = () => togglePopup('settings-sidebar', 'open');
document.getElementById('close-settings').onclick = () => togglePopup('settings-sidebar', 'close');
...
function playAudio(url, iconEl) {
    ...
    document.querySelectorAll('.q-icon-btn.fa-pause').forEach(i => i.classList.replace('fa-pause', 'fa-play'));
...
async function fetchSurahData(surahNumber, englishName) {
    ...
    try {
        ...
        const response = await fetch(`https://api.alquran.cloud/v1/surah/${surahNumber}/editions/quran-uthmani,en.ahmedali,ar.alafasy`);
...
let html = `
    <div class="q-header" style="text-align: center; padding: 40px 0; border-bottom: 1px solid #eee;">
        <div class="q-arabic-title" style="font-family: 'Amiri', serif; font-size: 48px; color: white;">${arabic.name}</div>
        ...
        <div class="q-translation-title" style="color: white; margin-top: 5px;">${arabic.englishNameTranslation}</div>
    </div>`;
...
arabic.ayahs.forEach((ayah, index) => {
    ...
    <div class="q-ayah-box" id="ayah-${index + 1}" style="border-bottom: 1px solid #eee; padding: 30px 0;">
        <div class="q-actions-row" style="display: flex; gap: 15px; align-items: center; margin-bottom: 20px; color: white;">
        ...
        <div class="q-text-content" style="text-align: right; margin-bottom: 25px;">
            <div class="q-arabic-line" style="font-family: 'Amiri', serif; font-size: 32px; line-height: 2; color: white; direction: rtl;">
                ${ayah.text} <span class="q-ayah-num" style="font-size: 22px; color: #666;">(${index + 1})</span>
            </div>
            <div class="q-english-line" style="text-align: left; color: #dfdfdf; font-size: 17px; line-height: 1.6; margin-top: 15px; font-family: sans-serif;">
                ${translation.ayahs[index].text}
            </div>
        </div>

        <div class="q-footer-tools" style="display: flex; gap: 25px; color: #dfdfdf; font-size: 14px; padding-top: 10px; font-family: sans-serif;">
        ...
        // Verses Loop (Matching your requested UI)
        arabic.ayahs.forEach((ayah, index) => {
            html += `
            <div class="q-ayah-box" id="ayah-${index + 1}" style="border-bottom: 1px solid #eee; padding: 30px 0;">
                <div class="q-actions-row" style="display: flex; gap: 15px; align-items: center; margin-bottom: 20px; color: #777;">
                    <span class="q-id" style="font-weight: bold; font-size: 14px;">${surahNumber}:${index + 1}</span>
                    <i class="fa-solid fa-play q-icon-btn" style="cursor:pointer;" onclick="playAudio('${audio.ayahs[index].audio}')"></i>
                    <i class="fa-regular fa-bookmark q-icon-btn" style="cursor:pointer;"></i>
                    
                    <div style="margin-left: auto; display: flex; gap: 18px;">
                        <i class="fa-regular fa-copy q-icon-btn" style="cursor:pointer;"></i>
                        <i class="fa-solid fa-share-nodes q-icon-btn" style="cursor:pointer;"></i>
                        <i class="fa-regular fa-pen-to-square q-icon-btn" style="cursor:pointer;"></i>
                        <i class="fa-solid fa-ellipsis q-icon-btn" style="cursor:pointer;"></i>
                    </div>
                </div>
                ...
                <div class="q-footer-tools" style="display: flex; gap: 25px; color: #666; font-size: 14px; padding-top: 10px; font-family: sans-serif;">
                    <span style="cursor:pointer;"><i class="fa-solid fa-book-open" style="margin-right:5px;"></i> Tafsirs</span>
                    <span style="cursor:pointer;"><i class="fa-solid fa-graduation-cap" style="margin-right:5px;"></i> Lessons</span>
                    <span style="cursor:pointer;"><i class="fa-regular fa-comments" style="margin-right:5px;"></i> Reflections</span>
                    <span style="cursor:pointer;"><i class="fa-regular fa-lightbulb" style="margin-right:5px;"></i> Answers</span>
                    <span style="cursor:pointer;"><i class="fa-solid fa-book" style="margin-right:5px;"></i> Hadith</span>
                </div>
            </div>`;
        });
    ...
    try {
        ...
        if (surahParam) {
            ...
            if (found) {
                ...
                document.querySelector('.surah-select span').innerText = `${sNum}. ${found.englishName}`;
            }
        } 
        else if (juzParam) {
        ...
        document.querySelector('.q-wrapper').innerHTML = `<div style="text-align:center; padding:50px; color:red;">Erreur de chargement. Vérifiez votre connexion.</div>`;
    }
}
...
function displayList(items, type) {
    ...
    items.forEach(item => {
        const div = document.createElement('div');
        div.className = 'search-item';
        div.style.cssText = "display:flex; justify-content:space-between; align-items:center; padding:12px; cursor:pointer; border-bottom:1px solid #f3f4f6;";
...
async function fetchByEndpoint(endpoint, title) {
    ...
    const wordsHtml = words.map((word, wordIndex) => {
        ...
        const s = ayah.surah ? ayah.surah.number : (typeof surahNumber !== 'undefined' ? surahNumber : 1);
    ...
    <div class="q-ayah-box" style="border-bottom: 1px solid #eee; padding: 30px 0;">
        <div class="q-text-content" style="text-align: right;">
            <div class="q-arabic-line" style="font-family: 'Amiri', serif; font-size: 32px; line-height: 2.2; color: white; direction: rtl;">
...
function playWordAudio(url, element) {
    ...
    audio.onended = () => {
        ...
        contentArea.innerHTML = html;
        if(window.innerWidth < 768) document.getElementById('surah-sidebar-nav').classList.remove('open-flex');
    } catch (e) {
...
async function showExtra(type, surah, ayah, containerId) {
    ...
    try {
        if(type === 'tafsir') {
            ...
            const resAr = await fetch(`https://api.alquran.cloud/v1/ayah/${surah}:${ayah}/ar.jalalayn`);
            const dataAr = await resAr.json();
            // Fetch English Tafsir
            const resEn = await fetch(`https://api.alquran.cloud/v1/ayah/${surah}:${ayah}/en.asad`);
            const dataEn = await resEn.json();
            
            content = `<strong>Tafsir (AR):</strong> ${dataAr.data.text}<br><br><strong>Note (EN):</strong> ${dataEn.data.text}`;
        } else if(type === 'lessons') {
            content = "Fi had l-ayah, nastafeed ana... (Hna ymken t-hardcodi chi logic aw tjibha men database dyalk)";
        } else if(type === 'hadith') {
...
function checkUrlParameters() {
    ...
    if (surahId) {
        ...
        if (allSurahs && allSurahs.length >= sNum) {
            ...
            // Kan-bddlo l-header
            document.querySelector('.surah-select span').innerText = `${sNum}. ${surahData.englishName}`;
...
document.getElementById('reciter-trigger').onclick = () => togglePopup('reciter-sidebar', 'open');
document.getElementById('close-reciters').onclick = () => togglePopup('reciter-sidebar', 'close');
...
document.getElementById('btn-listen').onclick = async () => {
    ...
    try {
        ...
        const res = await fetch(`https://api.alquran.cloud/v1/surah/${surahNum}/${selectedReciterId}`);
        const data = await res.json();
        const ayahs = data.data.ayahs;
        ...
        document.getElementById('current-reciter-name').innerText = selectedReciterName;
        document.getElementById('current-surah-name').innerText = data.data.englishName;
...
function stopPlayback() {
    ...
    document.getElementById('btn-listen').innerHTML = `<i class="fas fa-play"></i> Listen`;
}
...
contentArea.addEventListener('scroll', function() {
    ...
    if (scrollTop > lastScrollTop && scrollTop > 50) {
        // Scroll l-taht: Khbi l-header o tle3 l-ktaba
        header.classList.add('header-hidden');
        contentArea.classList.add('content-expanded');
    } else if (scrollTop < lastScrollTop) {
        // Scroll l-foug: Rje3 l-header o hbet l-ktaba
